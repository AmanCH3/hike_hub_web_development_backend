...
// import adminApi from '../api/adminApi'; // Import the new adminApi instance

// //-///////////////////////////////////////////////////////////////////////////
// // API SERVICE FUNCTIONS - Replace mock data with API calls
// //-///////////////////////////////////////////////////////////////////////////
...
//     return data;
// };
// const addProduct = (newProduct) => adminApi.post('/admin/products', newProduct);
// const updateProduct = (updatedProduct) => adminApi.put(`/admin/products/${updatedProduct.id}`, updatedProduct);
// const deleteProduct = (productId) => adminApi.delete(`/admin/products/${productId}`);
...
//     return data;
// };
// const addCategory = (newCategory) => adminApi.post('/admin/categories', newCategory);
// const updateCategory = (updatedCategory) => adminApi.put(`/admin/categories/${updatedCategory.id}`, updatedCategory);
// const deleteCategory = (categoryId) => adminApi.delete(`/admin/categories/${categoryId}`);
...
//     return data;
// };
// const updateOrderStatus = ({ orderId, status }) => adminApi.patch(`/admin/orders/${orderId}`, { status });
...
// //-///////////////////////////////////////////////////////////////////////////
// // PAGE COMPONENTS (Updated with Data Fetching)
// //-///////////////////////////////////////////////////////////////////////////
...
//                             <p className="text-2xl font-bold text-gray-800 dark:text-white">₹{stats.totalRevenue?.toLocaleString() || 0}</p>
//                         </div>
//                     </div>
//                 </Card>
...
//                             <p className="text-2xl font-bold text-gray-800 dark:text-white">{stats.totalCustomers || 0}</p>
//                         </div>
//                     </div>
//                 </Card>
//             </div>
...
//                         <ResponsiveContainer>
//                             <PieChart>
//                                 <Pie data={stats.topProductsData} dataKey="sales" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
//                                     {stats.topProductsData.map((entry, index) => <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}
//                                 </Pie>
...
//                                             className={`w-full p-1.5 text-xs rounded-md border-gray-300 dark:border-gray-600 dark:text-white focus:ring-blue-500 ${getStatusColor(order.status).replace('text-', 'dark:text-').replace('bg-', 'dark:bg-')}`}
//                                             >
...
//     const addMutation = useMutation({ mutationFn: addProduct, onSuccess: () => queryClient.invalidateQueries(['products']) });
//     const updateMutation = useMutation({ mutationFn: updateProduct, onSuccess: () => queryClient.invalidateQueries(['products']) });
...
//                                     <td className="p-3 text-gray-600 dark:text-gray-300">{category ? category.name : 'Uncategorized'}</td>
...
//     const updateMutation = useMutation({ mutationFn: updateCategory, onSuccess: () => queryClient.invalidateQueries(['categories']) });
...
//         const dataToSave = editingCategory ? { ...editingCategory, ...categoryData } : categoryData;
...
//                     <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
//                     <input type="text" placeholder="Search by name or email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
//                 </div>
//                 <div className="overflow-x-auto">
//                     <table className="w-full text-left">
//                          <thead className="text-sm text-gray-500 dark:text-gray-400 uppercase bg-gray-50 dark:bg-gray-700">
//                             <tr>
//                                 <th className="p-3">Name</th><th className="p-3">Email</th><th className="p-3">Joined On</th>
//                             </tr>
//                         </thead>
//                         <tbody>
...
//     useEffect(() => {
//         if(product) { setFormData({ id: product._id, ...product}); } 
//         else { setFormData({ name: '', categoryId: '', price: '', stock: '', imageUrl: ''}); }
//     }, [product]);
...
//                 </div>
//                 <Input id="imageUrl" name="imageUrl" label="Product Image URL" value={formData.imageUrl} onChange={handleChange} />
...
//         e.preventDefault();
//         const dataToSave = category ? { id: category._id, name } : { name };
//         onSave(dataToSave);
//     };
...
// //-///////////////////////////////////////////////////////////////////////////
// // MAIN APP COMPONENT & LAYOUT (No major changes here)
// //-///////////////////////////////////////////////////////////////////////////
...
//                 window.location.href = '/'; // Or trigger navigate context if available
//                 return null;
...
// // Hardcoding the API URL to resolve build-time environment variable issues.
...
//     return adminApi.put(`/products/${updatedProduct.id}`, updatedProduct);
// };
...
//     return adminApi.put(`/categories/${updatedCategory.id}`, updatedCategory);
// };
...
//     // Default empty data to prevent crashes if API returns partial or null data
//     const safeStats = {
//         totalRevenue: 0,
//         totalOrders: 0,
//         totalCustomers: 0,
...
//     // FIXED: Check if data is an array before filtering. It might be an object like { users: [...] }.
...
//     useEffect(() => {
//         if(product) { setFormData({ id: product._id, name: product.name, categoryId: product.categoryId, price: product.price, stock: product.stock, imageUrl: product.imageUrl }); } 
//         else { setFormData({ name: '', categoryId: '', price: '', stock: '', imageUrl: ''}); }
//     }, [product, isOpen]); // Rerun effect when modal opens
...
//         if(category) { setName(category.name); } 
//         else { setName(''); }
//     }, [category, isOpen]); // Rerun effect when modal opens
...
const fetchUsers = async () => {
    const { data } = await adminApi.get('/admin/users');
    // CORRECTED: Return the nested 'data' array from the response object
    return data.data;
};
...
const ProductsPage = () => {
    ...
    const handleSave = (productData) => {
        // CORRECTED: Create a new object for the payload and fix field names
        const dataToSend = {
            ...productData,
            category: productData.categoryId, // Rename categoryId to category
        };
        delete dataToSend.categoryId; // Remove the old key

        // CORRECTED: Add validation for required imageUrl field
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {products.length > 0 ? (
                                products.map(product => {
                                    const category = categories?.find(c => c._id === product.category); // Updated to check product.category
                                    return (
                                    <tr key={product._id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/50">
                                        <td className="p-3">
                                            <img src={product.imageUrl || 'https://placehold.co/100x100/CCCCCC/FFFFFF?text=No+Image'} alt={product.name} className="w-12 h-12 object-cover rounded-md"/>
...
const ProductFormModal = ({ isOpen, onClose, onSave, product, categories }) => {
    ...
    useEffect(() => {
        if (product) { 
            setFormData({ 
                name: product.name || '', 
                categoryId: product.categoryId || product.category || '', // Handles both cases
                price: product.price || '', 
                stock: product.stock || '', 
